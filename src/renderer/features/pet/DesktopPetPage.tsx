...
export const DesktopPetPage = () => {
  ...
  const [currentAvatar, setCurrentAvatar] = useState<string>(mascotImage);
  ...
  useEffect(() => {
    ...
    const off = window.assistantApi.pet.onEmotion((emotion) => {
      const nextAvatar = PET_FACE_BY_EMOTION[emotion] ?? mascotImage;
      switchAvatar(nextAvatar);
      if (resetTimer) window.clearTimeout(resetTimer);
      resetTimer = window.setTimeout(() => switchAvatar(mascotImage), EMOTION_RESET_MS);
    });
    return () => {
      off();
      if (resetTimer) window.clearTimeout(resetTimer);
