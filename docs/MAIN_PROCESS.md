...
| 文件 | 作用 | 常见修改 |
| --- | --- | --- |
| `src/main-process/llm.service.ts` | OpenAI 兼容 `POST /chat/completions` | 换路径、加 `max_tokens`、默认 `temperature` |
...
**统一注册处：** `src/main-process/ipc/register.ts`

- 所有 `ipcMain.handle('xxx', …)` 都在这里。
